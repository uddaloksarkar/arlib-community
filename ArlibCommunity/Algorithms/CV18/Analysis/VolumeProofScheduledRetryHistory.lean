...
noncomputable def scheduledBalancedAccuracyTransitionCollectLaw
  ...
  | samples + 1, total, current =>
      (scheduledBalancedAccuracyTransitionLawAux q I sigma2 proposalCap properStride
...
theorem scheduledBalancedAccuracyTransitionCollectLaw_measurable_and_probability
    ...
    ∀ samples,
      Measurable (fun state : ℝ × AmbientSpace q.n =>
        scheduledBalancedAccuracyTransitionCollectLaw q I sigma2 weight proposalCap
...
noncomputable def scheduledBalancedAccuracyTransitionCollectLawFirst
    (q : VolumeParams) (I : VolumeInput q.n) (sigma2 : ℝ)
    (weight : AmbientSpace q.n → ℝ)
...
theorem scheduledBalancedAccuracyTransitionCollectLawFirst_full
    (q : VolumeParams) (I : VolumeInput q.n) (sigma2 : ℝ)
    (weight : AmbientSpace q.n → ℝ)
    (proposalCap properStride retryLimit samples : ℕ)
    (total : ℝ) (current : AmbientSpace q.n) :
    scheduledBalancedAccuracyTransitionCollectLawFirst q I sigma2 weight proposalCap
...
theorem map_scheduledBalancedRetryCollectLawAux_eq_transitionCollectLawFirst
    (q : VolumeParams) (I : VolumeInput q.n)
    {sigma2 : ℝ} (hsigma2 : 0 < sigma2)
    {weight : AmbientSpace q.n → ℝ} (hweight : Measurable weight)
    (proposalCap properStride retryLimit : ℕ) :
    ∀ samples attempts total current,
      (balancedRetryCollectLawAux
        ...
        scheduledBalancedAccuracyTransitionCollectLawFirst q I sigma2 weight
          proposalCap properStride retryLimit attempts samples total current := by
  letI : Fact (0 < sigma2) := ⟨hsigma2⟩
  let B := scheduledBalancedAccuracyRetryBlockKernel q I sigma2 proposalCap properStride
  ...
  | h samples ihSamples =>
      ...
      | zero =>
          ...
          simp only [balancedRetryCollectLawAux,
            scheduledBalancedAccuracyTransitionCollectLawFirst]
      ...
      | succ future =>
          ...
          | zero =>
              ...
              simp only [balancedRetryCollectLawAux,
                scheduledBalancedAccuracyTransitionCollectLawFirst,
                scheduledBalancedAccuracyTransitionLawAux]
              rw [Measure.map_dirac' hOut]
              ...
              have htail : Measurable tail := by
                dsimp only [tail]
                have hcollect :=
                  (scheduledBalancedAccuracyTransitionCollectLaw_measurable_and_probability
          ...
          | succ attempts ihAttempts =>
              ...
              have hright :
                ...
                rw [scheduledBalancedAccuracyTransitionCollectLawFirst]
                rw [scheduledBalancedAccuracyTransitionLawAux]
                let expanded : Option (ℝ × AmbientSpace q.n) →
                    Measure (Option (AmbientSpace q.n)) := fun block =>
                  match block with
                  | none => Measure.dirac none
              ...
              | some block =>
                  ...
                  have hsecond : ∀ᵐ result ∂R mixed,
                    ...
                    scheduledBalancedAccuracyGaussianRejectionLaw_ae_snd
                      q I sigma2 mixed
                  ...
                  · have hc0 : accuracyScaleFactor q ≠ 0 :=
                    ...
                    calc
                      Measure.map Out
                          (balancedRetryCollectLawAux B R weight retryLimit
                            retryLimit future (total + weight result.2) mixed) =
                        scheduledBalancedAccuracyTransitionCollectLawFirst q I sigma2
                          weight proposalCap properStride retryLimit retryLimit
                          future (total + weight result.2) mixed :=
                            ihSamples future (by omega) retryLimit
                              (total + weight result.2) mixed
                      _ = scheduledBalancedAccuracyTransitionCollectLaw q I sigma2
                            weight proposalCap properStride retryLimit future
                            (total + weight result.2) mixed :=
                        scheduledBalancedAccuracyTransitionCollectLawFirst_full
...
theorem scheduledBalancedAccuracyRetryCollectLaw_eq_transitionCollectLaw
    (q : VolumeParams) (I : VolumeInput q.n)
    {sigma2 : ℝ} (hsigma2 : 0 < sigma2)
    {weight : AmbientSpace q.n → ℝ} (hweight : Measurable weight)
    (proposalCap properStride retryLimit samples : ℕ)
    (current : AmbientSpace q.n) :
    scheduledBalancedAccuracyRetryCollectLaw q I sigma2 weight proposalCap properStride
        retryLimit samples current =
      scheduledBalancedAccuracyTransitionCollectLaw q I sigma2 weight proposalCap
        properStride retryLimit samples 0 current := by
  unfold scheduledBalancedAccuracyRetryCollectLaw
  rw [map_scheduledBalancedRetryCollectLawAux_eq_transitionCollectLawFirst
    q I hsigma2 hweight]
  exact scheduledBalancedAccuracyTransitionCollectLawFirst_full q I sigma2 weight
...
theorem scheduledBalancedAccuracyRetryCollect_runEstimate_eq_transitionCollectLaw
    ...
    (scheduledBalancedAccuracyRetryCollect q sigma2 weight proposalCap properStride
      ...
      scheduledBalancedAccuracyTransitionCollectLaw q I sigma2 weight proposalCap
        properStride retryLimit samples 0 current := by
  rw [scheduledBalancedAccuracyRetryCollect_semantics q I oracle hsigma2 hweight]
  exact scheduledBalancedAccuracyRetryCollectLaw_eq_transitionCollectLaw
    q I hsigma2 hweight proposalCap properStride retryLimit samples current

#print axioms scheduledBalancedAccuracyTransitionCollectLaw_measurable_and_probability
#print axioms map_scheduledBalancedRetryCollectLawAux_eq_transitionCollectLawFirst
#print axioms scheduledBalancedAccuracyRetryCollectLaw_eq_transitionCollectLaw
#print axioms scheduledBalancedAccuracyRetryCollect_runEstimate_eq_transitionCollectLaw

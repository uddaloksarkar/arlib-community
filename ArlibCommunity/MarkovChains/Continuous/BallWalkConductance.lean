...
* `theorem2_of_lazy_ballWalk` — the same, composed with
  `ArlibCommunity.PointwiseRoute.theorem2_of_mixesWithin`: from an isoperimetric inequality for `K`
...
`ArlibCommunity.PointwiseRoute.theorem2_of_mixesWithin` consumes a chain that has mixed in
...
theorem theorem2_of_lazy_ballWalk (hn : 1 ≤ n)
  ...
  exact ArlibCommunity.PointwiseRoute.theorem2_of_mixesWithin hK hK0 hKtop hV hacc.le

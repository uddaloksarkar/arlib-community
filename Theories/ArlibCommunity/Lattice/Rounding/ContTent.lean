...
-/
import ArlibCommunity.Lattice.Rounding.KernelIntegral
import ArlibCommunity.Lattice.KV97.Escape
...
`ArlibCommunity.Lattice.Rounding.RoundLaw` builds the *lattice-variable* reading of the
same kernel (a two-point law on `ℤ` per coordinate) and
`ArlibCommunity.Lattice.Rounding.ProdKernel` builds the *continuous-variable* density
`ArlibCommunity.Lattice.Rounding.prodTent`. This file builds the **continuous law itself**:

* `ArlibCommunity.Lattice.Rounding.tentReal` — Lebesgue measure on `ℝ` weighted by `tent`;
* `ArlibCommunity.Lattice.Rounding.tentPi n` — its `n`-fold product on `Fin n → ℝ`, the law
...
the continuous escape bound `ArlibCommunity.Lattice.Rounding.tentPi_escape_prob_le`.
...
(`ArlibCommunity.Lattice.Rounding.setIntegral_prodTent_eq_tentPi_real`), and hence bounds
the **escape mass** of the kernel outside the inflated polytope,

  `∫ p in (Polytope.inflate A b κ)ᶜ, prodTent x p ≤ r · exp(−κ²/2)`

(`ArlibCommunity.Lattice.Rounding.setIntegral_prodTent_compl_inflate_le`).

## The bridge between `Measure.pi` and `withDensity`

The one genuinely technical step is
`ArlibCommunity.Lattice.Rounding.tentPi_eq_withDensity`:
...
`ArlibCommunity.Lattice.Rounding.lintegral_pi_prod_indicator_tent`.
-/
...
`ArlibCommunity.Lattice.Rounding.integral_tent`, obtained through
...
/-- `ArlibCommunity.Lattice.Rounding.tentReal` is a probability measure. -/
...
/-- `ArlibCommunity.Lattice.Rounding.tentReal` of a measurable set is the tent's integral
over it. -/
...
(`ArlibCommunity.Lattice.Rounding.integral_id_mul_tent`). -/
...
outside (`ArlibCommunity.Lattice.Rounding.tent_eq_zero_of_notMem`), so the complement is
null. -/
...
transfers coordinatewise exactly as in `ArlibCommunity.Lattice.Rounding.RoundLaw`: the
...
(compare `ArlibCommunity.Lattice.Rounding.iIndepFun_latDisp`, the discrete analogue). -/
...
the discrete instance built from `ArlibCommunity.Lattice.Rounding.roundLaw`.
-/
...
hypotheses are `ArlibCommunity.Lattice.Rounding.{measurable_tentPi_coord, iIndepFun_tentPi,
...
`ArlibCommunity.Lattice.Rounding.lintegral_ofReal_pi_prod`. -/
...
/-- The real-valued form of `ArlibCommunity.Lattice.Rounding.tentPi_apply`. -/
...
`ArlibCommunity.Lattice.Rounding.tentPi_escape_prob_le` this bounds the **escape mass** of
...
under. What is left is `ArlibCommunity.Lattice.Rounding.tentPi_real_apply`. -/
...
`ArlibCommunity.Lattice.Rounding.setIntegral_prodTent_compl` — which says the mass the
kernel puts on `inflate A b κ` is `1` minus this escape mass — it is exactly the
lower bound `Pr[rnd p = x] ≥ (1 − r·exp(−κ²/2)) / Vol(P')`.

Proof: `ArlibCommunity.Lattice.Rounding.setIntegral_prodTent_eq_tentPi_real` turns the set
integral into the probability that `x + Y` escapes the inflation, and
`ArlibCommunity.Lattice.Rounding.tentPi_escape_prob_le` bounds that. -/

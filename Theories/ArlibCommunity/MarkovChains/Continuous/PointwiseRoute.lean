...
-/
import Arlib.MarkovChains.Continuous.PointwiseMixing
import Arlib.MarkovChains.Continuous.MixingFromConductance
import Arlib.Probability.UniformOn
import ArlibCommunity.Lattice.KV97.Theorem2
...
(`Arlib.MarkovChains.Continuous.not_exists_pointwiseClose_of_tvLe`) — and that the ball walk is not
...
useless.  The whole argument is one line of algebra,
`ArlibCommunity.MarkovChains.Continuous.PointwiseRoute.window_of_tvLe`:
...
`ArlibCommunity.KV97.PointwiseAlmostUniform` both presuppose the sampler's law **has a density**.  A
ball-walk law started at a point does not: the holding term `(1 − ℓ(x))·δ_x` of
`Arlib.MarkovChains.Continuous.ballWalk_apply_set` keeps an atom of mass `∏(1 − ℓ)` forever.
...
`pointwiseAlmostUniform_of_rejection` hands the result straight to `ArlibCommunity.KV97.theorem2` at
`ε = 0`.
...
`Arlib.uniformOn`, `Arlib.condOn`, `Arlib.MarkovChains.Continuous.PointwiseClose` and
`ArlibCommunity.KV97.PointwiseAlmostUniform` as they already stand; nothing here defines a class
...
`ArlibCommunity.KV97.theorem2` is no longer needed at all.
-/

namespace ArlibCommunity.MarkovChains.Continuous.PointwiseRoute
...
`N/V`, i.e. a relative `θ`.  This is `ArlibCommunity.KV97.cor_linf`'s conclusion obtained without any
...
`ArlibCommunity.KV97.theorem2` takes a *density* `f` and computes `Pr[rnd p = x]` as
...
it — that lemma transfers `[0,1]`-valued observables, and `ArlibCommunity.Lattice.Rounding` records
...
with `E` the escape mass — **exactly the shape `ArlibCommunity.KV97.theorem2` produces from its
...
no density: `Arlib.MarkovChains.Continuous.MixesWithin` is exactly what a conductance bound delivers,
...
theorem theorem2_of_mixesWithin {S : Set (EuclideanSpace ℝ (Fin n))}
    ...
    (hmix : Arlib.MarkovChains.Continuous.MixesWithin P (uniformOn volume S) μ₀ t
      (ENNReal.ofReal (θ / V)))
    (hesc : ∫ p in Sᶜ, prodTent x p ≤ E) :
    (1 - (E + θ)) / V ≤ ∫ p, prodTent x p ∂(Arlib.MarkovChains.Continuous.iterate P μ₀ t) ∧
      ∫ p, prodTent x p ∂(Arlib.MarkovChains.Continuous.iterate P μ₀ t) ≤ (1 + θ) / V :=
...
mixing time is *logarithmic* in the tolerance: `Arlib.MarkovChains.Continuous.conductanceMixingTime_le`
...
theorem conductanceMixingTime_inv_volume_le {M phi V θ : ℝ} (hM : 1 ≤ M) (hphi0 : 0 < phi)
    ...
    (Arlib.MarkovChains.Continuous.conductanceMixingTime M phi (θ / V) : ℝ)
      ≤ (Real.log M + 2 * Real.log V + 2 * Real.log (1 / θ)) / phi ^ 2 + 1 := by
  have h := Arlib.MarkovChains.Continuous.conductanceMixingTime_le hM hphi0 (by positivity) hle
  refine h.trans_eq ?_
...
`Arlib.MarkovChains.Continuous.PointwiseClose` and `ArlibCommunity.KV97.PointwiseAlmostUniform` both require the
sampler's law to *have a density*.  A ball walk started at a point never does: the holding
term `(1 − ℓ(x)) · δ_x` of `Arlib.MarkovChains.Continuous.ballWalk_apply_set` leaves an atom of mass
...
`Arlib.MarkovChains.Continuous.PointwiseClose μ ν ε` fails for every `ε` — `ε = ⊤` included. -/
theorem not_pointwiseClose_of_atom {Ω : Type*} [MeasurableSpace Ω] {μ ν : Measure Ω}
    {x₀ : Ω} (hν : ν {x₀} = 0) (hμ : μ {x₀} ≠ 0) (ε : ℝ≥0∞) :
    ¬ Arlib.MarkovChains.Continuous.PointwiseClose μ ν ε := fun h => hμ (h.1 hν)
...
theorem not_pointwiseClose_uniformOn_of_atom {Ω : Type*} [MeasurableSpace Ω]
    ...
    (ε : ℝ≥0∞) : ¬ Arlib.MarkovChains.Continuous.PointwiseClose μ (uniformOn ν S) ε :=
...
Together with `Arlib.MarkovChains.Continuous.not_exists_pointwiseClose_of_tvLe` this settles the
...
theorem exists_tvLe_with_atom {Ω : Type*} [MeasurableSpace Ω] (ν : Measure Ω)
    ...
    ∃ μ : Measure Ω, IsProbabilityMeasure μ ∧ TVLe μ ν δ ∧
      ∀ ε : ℝ≥0∞, ¬ Arlib.MarkovChains.Continuous.PointwiseClose μ ν ε := by
...
`ArlibCommunity.KV97.theorem2` holds at `ε = 0` and nothing has to be approximated at all.
...
hypothesis `Arlib.MarkovChains.Continuous.PointwiseClose` — and the reason route 2 is worth stating
...
theorem pointwiseClose_condOn_uniformOn (μ : Measure Ω) {B S : Set Ω} (hS : MeasurableSet S)
    ...
    Arlib.MarkovChains.Continuous.PointwiseClose (condOn (uniformOn μ B) S) (uniformOn μ S) 0 := by
  ...
  exact Arlib.MarkovChains.Continuous.pointwiseClose_self _
...
quantity of `ArlibCommunity.KV97.theorem2` are the same number.  Unconditional, by
...
/-- **Route 2 discharges `ArlibCommunity.KV97.theorem2`'s hypothesis outright.**

Rejecting a uniform draw from a superset `B ⊇ P'` onto `P'` gives a law under which
`Pr[rnd p = x]` is *literally* the density-form integrand of `ArlibCommunity.KV97.theorem2` at the
constant density `1/V`, and that density is `ArlibCommunity.KV97.PointwiseAlmostUniform` at error
...
theorem pointwiseAlmostUniform_of_rejection {n : ℕ}
    ...
    ∫ p, ArlibCommunity.Lattice.Rounding.prodTent x p ∂(condOn (uniformOn volume B) S)
        = ∫ p in S, (1 / V) * ArlibCommunity.Lattice.Rounding.prodTent x p ∧
      ArlibCommunity.KV97.PointwiseAlmostUniform (fun _ => 1 / V) S V ε := by
  refine ⟨?_, ArlibCommunity.KV97.PointwiseAlmostUniform.uniform S hV hε⟩
...
theorem exists_theorem2_of_tvLe_witness (n : ℕ) (x : EuclideanSpace ℝ (Fin (n + 1)))
    ...
    ∃ (S : Set (EuclideanSpace ℝ (Fin (n + 1))))
      ...
      IsProbabilityMeasure μ ∧ MeasurableSet S ∧ volume S ≠ 0 ∧ volume S ≠ ⊤ ∧
        ...
        (∫ p in Sᶜ, ArlibCommunity.Lattice.Rounding.prodTent x p) ≤ E ∧
        ((1 - (E + θ)) / V ≤ ∫ p, ArlibCommunity.Lattice.Rounding.prodTent x p ∂μ ∧
          (∫ p, ArlibCommunity.Lattice.Rounding.prodTent x p ∂μ) ≤ (1 + θ) / V) := by
  ...
  refine ⟨S, uniformOn volume S, (volume S).toReal,
    ∫ p in Sᶜ, ArlibCommunity.Lattice.Rounding.prodTent x p, inferInstance, hS, h0, htop, rfl, hV,
...
end ArlibCommunity.MarkovChains.Continuous.PointwiseRoute
